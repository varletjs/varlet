...
export function resolveMobileSiteRoutePath(
...
export function resolvePcSiteRoutePath(
  mdPath: string,
...
export async function findDocsMDPaths(): Promise<I18MDPath> {
  ...
  return srcDir
    ...
    .reduce((i18MdPath: I18MDPath, componentName: string) => {
      ...
      isDir(docsPath) && (i18MdPath[componentName] = readdirSync(docsPath).map((filename: string) => slash(resolve(docsPath, filename))))
      return i18MdPath
    }, {})
}
...
export async function buildMobileSiteRoutes() {
  if (!pathExistsSync(SRC_DIR)) {
    await writeFile(SITE_MOBILE_ROUTES, 'export default []')
    logger.warning('you need make a src directory in your workspace')
    return
  }
...
export async function buildPcSiteRoutes() {
  if (!pathExistsSync(SRC_DIR)) {
    await writeFile(SITE_PC_ROUTES, 'export default []')
    logger.warning('you need make a src directory in your workspace')
    return
  }
