...
`@varlet/cli` has built-in `single file component (sfc)` and `tsx` two styles of component library project templates, which can be directly generated by the `gen` command.
...
If you want to add other pages to the component library, you can write vue components in the creation `pages` folder under the project root directory to generate other pages
...
```shell
# playground-ignore
varlet-cli gen [-n | --name[=<componentName>]] [-s | --sfc] [-t | --tsx] [-l | --locale]
...
-s
--sfc
  Automatically generate .vue format project
-t
--tsx
  Automatically generate .tsx format project
-l
--locale
