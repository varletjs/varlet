...
type MaybePick<T extends object, U extends Extract<keyof T, string>> = Record<string, unknown> extends T
  ? Partial<Pick<T, U>>
  : Pick<T, U>
...
export function omit<T extends object, U extends Extract<keyof T, string>>(obj: T, exclude: U[]): Omit<T, U> {
  const clone = { ...obj }
...
export function searchItems<T extends any = any>(items: T[], search: string): T[] {
  ...
  return items.filter((item: any) =>
    Object.keys(item).some((key) => defaultFilter(getObjectValueByPath(item, key), search, item))
  )
}
