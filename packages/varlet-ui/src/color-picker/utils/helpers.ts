...
export const clamp = (value: number, min: number, max: number): number => min < max ? (value < min ? min : value > max ? max : value) : value < max ? max : value > min ? min : value
export const upperCase = (word: string): string => word.toLocaleUpperCase()
export const lowerCase = (word: string): string => word.toLocaleLowerCase()
