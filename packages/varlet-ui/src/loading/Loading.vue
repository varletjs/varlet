<template>
  <div :class="n()">
    <div
      :class="classes(n('content'), [loading, n('content--active')])"
      v-if="$slots.default"
    >
      <slot />
      <div
        :class="n('content-mask')"
        v-if="loading"
      />
    </div>
    <div
      :class="classes(n('$--box'), n('body'), [$slots.default, n('inside')])"
      v-if="isShow"
    >
      <div
        :class="n('circle')"
      ...
      <template
        v-for="(nums, key) in loadingTypeDict"
        :key="key"
      >
        <div
          :class="classes(n(key), n(`${key}--${size}`))"
          v-if="type === key"
        >
          <div
            v-for="num in nums"
