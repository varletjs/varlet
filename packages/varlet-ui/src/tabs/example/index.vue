<template>
  <app-type>基本使用</app-type>
  <var-tabs v-model:active="active">
    <var-tab>选项1</var-tab>
    <var-tab>选项2</var-tab>
    <var-tab>选项3</var-tab>
  </var-tabs>

  <app-type>修改样式</app-type>
  <var-tabs
    elevation
    color="#2979ff"
    active-color="#fff"
    inactive-color="hsla(0, 0%, 100%, .6)"
    v-model:active="active2"
  >
    <var-tab>选项1</var-tab>
    <var-tab>选项2</var-tab>
    <var-tab>选项3</var-tab>
  </var-tabs>

  <app-type>禁用标签</app-type>
  <var-tabs
    elevation
    color="#2979ff"
    active-color="#fff"
    inactive-color="hsla(0, 0%, 100%, .6)"
    v-model:active="active3"
  >
    <var-tab>选项1</var-tab>
    <var-tab>选项2</var-tab>
    <var-tab disabled>禁用选项</var-tab>
  </var-tabs>

  <app-type>自动滚动</app-type>
  <var-tabs
    elevation
    color="#2979ff"
    active-color="#fff"
    inactive-color="hsla(0, 0%, 100%, .6)"
    v-model:active="active4"
  >
    <var-tab>选项1</var-tab>
    <var-tab>选项2</var-tab>
    <var-tab>选项3</var-tab>
    <var-tab>选项4</var-tab>
    <var-tab>选项5</var-tab>
    <var-tab>选项7</var-tab>
    <var-tab>选项8</var-tab>
    <var-tab>选项9</var-tab>
  </var-tabs>

  <app-type>选项垂直布局</app-type>
  <var-tabs
    elevation
    item-direction="vertical"
    color="#2979ff"
    active-color="#fff"
    inactive-color="hsla(0, 0%, 100%, .6)"
    v-model:active="active5"
  >
    <var-tab>
      <var-icon class="icon" name="information" />
      <div>选项1</div>
    </var-tab>
    <var-tab>
      <var-icon class="icon" name="warning" />
      <div>选项2</div>
    </var-tab>
    <var-tab>
      <var-icon class="icon" name="error" />
      <div>选项3</div>
    </var-tab>
  </var-tabs>

  <app-type>容器垂直布局</app-type>
  <var-tabs
    class="vertical-tabs"
    elevation
    layout-direction="vertical"
    color="#2979ff"
    active-color="#fff"
    inactive-color="hsla(0, 0%, 100%, .6)"
    v-model:active="active6"
  >
    <var-tab>选项1</var-tab>
    <var-tab>选项2</var-tab>
    <var-tab>选项3</var-tab>
  </var-tabs>

  <app-type>视图联动</app-type>
  <var-tabs
    elevation
    color="#2979ff"
    active-color="#fff"
    inactive-color="hsla(0, 0%, 100%, .6)"
    v-model:active="activeRelation"
  >
    <var-tab>选项1</var-tab>
    <var-tab>选项2</var-tab>
    <var-tab>选项3</var-tab>
  </var-tabs>

  <var-tabs-items v-model:active="activeRelation">
    <var-tab-item style="padding: 10px 5px">
      呜啦啦啦火车笛，随着奔腾的马蹄。
      小妹妹吹着口琴，夕阳下美了剪影。
      我用子弹写日记，介绍完了风景。
      接下来换介绍我自己。
      我虽然是个牛仔，在酒吧只点牛奶。
      为什么不喝啤酒，因为啤酒伤身体。
    </var-tab-item>
    <var-tab-item style="padding: 10px 5px">
      很多人不长眼睛，嚣张都靠武器。
      赤手空拳就缩成蚂蚁。
      不用麻烦了，不用麻烦了。
      不用麻烦，不用麻烦了，不用麻烦了。
    </var-tab-item>
    <var-tab-item style="padding: 10px 5px">
      你们一起上，我在赶时间。
      每天决斗观众都累了，英雄也累了。
      不用麻烦了，不用麻烦了。
      副歌不长你们有几个，一起上好了。
      正义呼唤我，美女需要我。
      牛仔很忙的。
    </var-tab-item>
  </var-tabs-items>

  <app-type>开启粘性布局</app-type>
  <var-tabs
    sticky
    elevation
    color="#2979ff"
    active-color="#fff"
    inactive-color="hsla(0, 0%, 100%, .6)"
    :offset-top="50"
    v-model:active="active7"
  >
    <var-tab>选项1</var-tab>
    <var-tab>选项2</var-tab>
    <var-tab>选项3</var-tab>
  </var-tabs>

  <div style="height: 120vh"></div>
</template>

<script>
import AppType from '@varlet/cli/site/mobile/components/AppType'
import Tabs from '..'
import Tab from '../../tab'
import Icon from '../../icon'
import TabsItems from '../../tabs-items'
import TabItem from '../../tab-item'
import context from '../../context'
import { onUnmounted, ref } from 'vue'
import { watchPlatform } from '../../utils/components'

export default {
  name: 'TabsExample',
  components: {
    [Tabs.name]: Tabs,
    [Tab.name]: Tab,
    [TabsItems.name]: TabsItems,
    [TabItem.name]: TabItem,
    [Icon.name]: Icon,
    AppType
  },
  setup() {
    const active = ref(0)
    const active2 = ref(0)
    const active3 = ref(0)
    const active4 = ref(0)
    const active5 = ref(0)
    const active6 = ref(0)
    const active7 = ref(0)
    const activeRelation = ref(0)

    const prevTouchmoveForbid = context.touchmoveForbid
    watchPlatform((platform) => {
      if (platform === 'pc') {
        context.touchmoveForbid = false
      }
    })
    onUnmounted(() => {
      context.touchmoveForbid = prevTouchmoveForbid
    })

    return {
      active,
      active2,
      active3,
      active4,
      active5,
      active6,
      active7,
      activeRelation,
    }
  },
}
</script>

<style lang="less" scoped>
.vertical-tabs {
  width: 80px;
  height: 120px;
}

.icon {
  margin-bottom: 5px;
}
</style>
