...
| `resolve-type` | The file read result type, Can be set to `default` `file` `dataUrl` (`default`, the image type contains base64 and File object, other types contain only File object. `file`, which contains only File object. `dataUrl`, which contains base64 and File object) | _string_ | `default` |
